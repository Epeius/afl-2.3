...
EXP_ST u8 *in_dir,                    /* Input directory with test cases  */
          ...
          *sync_id,                   /* Fuzzer ID                        */
          *outer_file_dir,            /* Outer testcases directory        */
...
EXP_ST u8  skip_deterministic,        /* Skip deterministic stages?       */
	   ...
	   useOuterFiles;	      /* Use outer testcases like s2e     */
...
static FILE* afl_log_file;

static hashset_t* g_hashes;
...
static u8 init_filter(char* filepath, u8* filter, s32 len )
{
    ...
    if (fp <= 0) {
        char log_msg[512];
        sprintf(log_msg, "Cannot open %s.\n", filename);
        fputs(log_msg, afl_log_file);
        return 0;
    }

    char log_msg[512];
    sprintf(log_msg, "Start reading %s.\n", filename);
    fputs(log_msg, afl_log_file);
...
havoc_stage:
  ...
  for (stage_cur = 0; stage_cur < stage_max; stage_cur++) {
    ...
    u8 new_hash = 0;
    {
        ...
        // This is not very effective because it has little possibility to have hash collision.
        u32 index = 0;
        char interesting_offs[len];
        memset(interesting_offs, 0, len);
        for(; index < MIN(temp_len, len); index++) {
            if (filter[index]) {
                interesting_offs[index] = out_buf[index];
            }
        }
        u32 chsum = hash32(interesting_offs, len, HASH_CONST);
        if (!hashset_is_member(*g_hashes, chsum)) {
            hashset_add(*g_hashes, chsum);
            new_hash = 1;
        }
...
/* Grab test cases from other tools, like S2E. */
static void fetch_outer_files(char** argv) {
 
  DIR* od; // Outer Directory
  struct dirent* od_ent;
  
  od = opendir(outer_file_dir);
  if (!od) PFATAL("Unable to open '%s'", outer_file_dir);
  FILE* pLogfile = fopen("import.log", "a");

  while ((od_ent = readdir(od))) {
    
    u8* path;
    s32 fd;
    struct stat st;

    if (od_ent->d_name[0] == '.') continue; // take care of '.' and ".."

    /* OK, try this new test case. */
    path = alloc_printf("%s/%s", outer_file_dir, od_ent->d_name);
    ...
    if (st.st_size && st.st_size <= MAX_FILE) {
      ...
      // write to testcase and start running.
      write_to_testcase(mem, st.st_size);
      fault = run_target(argv);

      if (stop_soon) return;

      u8 res = save_if_interesting(argv, mem, st.st_size, fault);

      queued_imported += res;

      if(res) {
	 fprintf(pLogfile, "%s:queued_imported=%d\n", path, queued_imported);
      }
    ...
    s32 status = unlink(path);
    if (status) {
	PFATAL("unlink() failed!");
    }
    ck_free(path);
...
static void usage(u8* argv0) {
  ...
  SAYF("\n%s [ options ] -- /path/to/fuzzed_app [ ... ]\n\n"
       ...
       "  -U            - Use outer testcases that gernerated by tools like s2e\n\n"
...
int main(int argc, char** argv) {
  ...
  while ((opt = getopt(argc, argv, "+i:o:f:m:t:T:U:dnCB:S:M:x:Q")) > 0)
    ...
    switch (opt) {
      ...
      case 'U': /* Use outer testcass */

	if (useOuterFiles) FATAL("Multiple -U options not supported");
	useOuterFiles = 1;
	outer_file_dir = optarg;
	break;
...
find_queue_cur:
    ...
    if (!stop_soon && useOuterFiles) {
       fetch_outer_files(use_argv);
    }
